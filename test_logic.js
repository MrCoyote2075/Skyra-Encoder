...
const decodeSkyra = (code) => {
    let idChar = code.charAt(0);
    let core = code.slice(1);
    
    // THE BUG IS HERE IN THE USER'S PROVIDED CODE:
    // User added this to decode:
    /*
    let temp = "";
    for (let i = 0; i < token.length; i++) {
        if (token[i] >= 'a' && token[i] <= 'z') temp += token[i].toUpperCase();
    ...
    core = temp;
    */
    // Problem 1: 'token' is not defined in decodeSkyra.
    // Problem 2: Case swap should usually be at the END of decode if it was at the START of encode.

    // Let's try to simulate the encode steps in reverse order.

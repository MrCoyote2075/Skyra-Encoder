...
// --- Encoding / Decoding helpers (extracted for clarity) ---
...
// Decodes a Skyra code (expects leading provider digit)
export const decodeSkyra = (code) => {
    if (!code || code.length < 2) throw new Error('Invalid encoded format');
    const idChar = code.charAt(0);
    let core = code.slice(1);
    if (!/\d/.test(idChar) || !core) throw new Error('Invalid encoded format');
    ...
    // reverse Step 3: unswap
    core = unswapParts(core);

    // reverse Step 2: core = last + first + middle -> reconstruct original: first + middle + last
    const n = core.length;
    const last = core.charAt(0) || '';
    const first = core.charAt(1) || '';
    const middle = n > 2 ? core.slice(2) : '';
    const originalToken = first + middle + last;

    // Last Step: Reverse the Case Swap (Case swap is its own inverse)
    let finalToken = "";
    for (let i = 0; i < originalToken.length; i++) {
        if (originalToken[i] >= 'a' && originalToken[i] <= 'z') {
            finalToken += originalToken[i].toUpperCase();
        } else if (originalToken[i] >= 'A' && originalToken[i] <= 'Z') {
            finalToken += originalToken[i].toLowerCase();
        } else {
            finalToken += originalToken[i];
        }
    }

    const provMap = { 1: 'shorturl', 2: 'tinyurl', 3: 'bitly' };
    return { providerId: Number(idChar), provider: provMap[Number(idChar)] || 'unknown', decoded: finalToken };
};
    ...
    return (
        <div className="App">
            <header className="App-header">
                ...
                <h1>Skyra Encoder</h1>
                <p className="subtitle">Paste a short URL (shorturl, tinyurl, bitly) to encode and decode its token.</p>
            </header>
            ...
            <main className="card">
                <div className="input-group">
                    ...
                    <input
                        ...
                        placeholder="Paste short URL (e.g. https://shorturl.at/ho8zF)"
                    />
                </div>
